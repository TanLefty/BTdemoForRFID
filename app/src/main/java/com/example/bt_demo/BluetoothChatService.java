 ...
 *
 *
 * 这个类是管理蓝牙跟其他设备的连接工作，有一个线程去监听将要连接的连接。
 * 还会开启一个线程，跟设备进行通信。
 *
 */
public class BluetoothChatService {
    ...
    private ConnectThread mConnectThread;   //通讯线程
    private ConnectedThread mConnectedThread; //连接ted线程
    private int mState;

    // Constants that indicate the current connection state             //当前连接状态
    public static final int STATE_NONE = 0;       // we're doing nothing   //无操作状态
    public static final int STATE_LISTEN = 1;     // now listening for incoming connections  //监听状态
    public static final int STATE_CONNECTING = 2; // now initiating an outgoing connection  //连接状态
